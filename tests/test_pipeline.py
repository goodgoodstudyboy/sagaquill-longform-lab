...
class PipelineTests(unittest.TestCase):
    ...
    def test_pipeline_accepts_sparse_input_and_finishes_with_closure(self) -> None:
        ...
        try:
            ...
            quality_report = json.loads((temp_dir / "data" / "quality-report.json").read_text(encoding="utf-8"))
            self.assertIn(quality_report["status"], {"pass", "warn", "fail"})
            self.assertEqual(delivery_manifest["files"]["quality_report"], "quality-report.md")
            ...
            self.assertEqual(run_summary["quality_report_path"], "delivery/quality-report.md")
            self.assertIn(run_summary["quality_status"], {"pass", "warn", "fail"})
